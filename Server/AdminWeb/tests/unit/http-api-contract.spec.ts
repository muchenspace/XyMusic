...
describe("administrator HTTP API contract", () => {
  ...
  it("maps auth, setup, scraping, and binary media gateways", async () => {
    ...
    const search: TagSearchInput = { source: "smart", verbatim: true, query: "song" };
    ...
    const candidateDetail: TagCandidateDetailInput = { candidate, verbatim: true };
    const fields = { title: true, artist: true, album: true, year: true, genre: true, lyrics: true, cover: true, overwrite: false };
    const apply: ApplyTagInput = { expectedVersion: 2, candidate, verbatim: true, fields, writeBack: false, reason: "test" };
    ...
    const artistSearch: ArtistSearchInput = { source: "smart", query: "Artist", sources: ["qmusic"] };
    ...
    const artistBatch: CreateArtistArtworkBatchInput = { items: [{ artistId: "artist-1", expectedVersion: 3 }], options: { sources: ["qmusic"], overwrite: false, reason: "batch artist artwork" } };
