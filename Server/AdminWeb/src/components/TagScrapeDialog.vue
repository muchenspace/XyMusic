...
function resetCandidateDetail(): void {
  detailOpen.value = false;
  detailCandidate.value = undefined;
}

function showCandidateDetail(candidate: TagCandidate): void {
...
async function apply(): Promise<void> {
  ...
  loading.value = true; error.value = "";
  try {
    const result = await scraping.apply(trackId, { expectedVersion: props.expectedVersion, candidate: selected.value, verbatim: verbatim.value, fields: { ...fields }, writeBack: writeBack.value, reason: "" });
    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.65fr)_minmax(300px,0.75fr)]">
      <section class="overflow-hidden rounded-2xl border border-[var(--border)]">
        <div class="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
          <div><h3 class="font-bold">匹配候选</h3><p class="mt-0.5 text-xs text-[var(--muted)]">选择最符合当前曲目的元数据</p></div>
        ...
        <div v-if="candidates.length" class="grid min-h-[360px] max-h-[56vh] gap-3 overflow-y-auto p-4 md:grid-cols-2">
          <article
            ...
            class="flex min-w-0 flex-col overflow-hidden rounded-2xl border transition-colors"
            :class="selected === item ? 'border-violet-500 bg-violet-500/8 ring-1 ring-violet-500/20' : 'border-[var(--border)]'"
          >
            <div class="flex min-w-0 flex-1 gap-4 p-4">
              <img v-if="item.albumImg" :src="scraping.artworkUrl(item.albumImg)" class="h-20 w-20 shrink-0 rounded-xl object-cover" alt="候选封面" width="80" height="80" loading="lazy" decoding="async" />
              <span v-else class="grid h-20 w-20 shrink-0 place-items-center rounded-xl bg-[var(--surface-muted)] text-xs text-[var(--muted)]">无封面</span>
              <div class="min-w-0 flex-1">
                <p class="truncate text-base font-bold">{{ item.name }}</p>
                <p class="mt-1 truncate text-sm text-[var(--muted)]">{{ item.artist }}</p>
                <p class="mt-1 truncate text-xs text-[var(--muted)]">{{ item.album || '未知专辑' }}</p>
                <p class="mt-3 text-[10px] font-bold uppercase text-[var(--primary)]">{{ item.source }} · 匹配分 {{ item.score ?? '—' }}</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-2 border-t border-[var(--border)] p-3">
              <AppButton :variant="selected === item ? 'primary' : 'secondary'" :disabled="selected === item" @click="selectCandidate(item)">
                <template #icon><Check :size="15" /></template>
                {{ selected === item ? '已选用' : '选用' }}
              </AppButton>
              <AppButton @click="showCandidateDetail(item)">
