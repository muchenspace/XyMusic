...
const channelConcurrency = reactive<Record<TagSource, number>>({ qmusic: 128, netease: 256, kugou: 128 });
...
<template>
  <BaseDialog v-model="open" :title="`批量刮削 ${submittedCount || tracks.length} 首曲目`" description="匹配、歌词、封面和字段应用全部由后端执行，关闭窗口不会中止任务。" width="xl">
    <template v-if="!job">
      <div class="mt-4 grid gap-3 sm:grid-cols-3"><label v-for="source in visibleSourceOptions" :key="`concurrency-${source.value}`" class="ui-label">{{ source.label }} <input v-model.number="channelConcurrency[source.value]" class="ui-input mt-1" type="number" min="1" :max="source.value === 'qmusic' ? 512 : source.value === 'netease' ? 1024 : 128" /></label></div>
