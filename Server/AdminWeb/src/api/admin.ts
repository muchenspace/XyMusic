...
export const adminApi = {
  ...
  users: (params: ListQuery & { status?: string; role?: string }, signal?: AbortSignal) => {
    return apiRequest<PageResult<UserSummary>>("/api/v1/admin/users", { query: query({ page: params.page, pageSize: params.pageSize, query: params.search, status: params.status, role: params.role, cursor: params.cursor, cursorMode: params.cursorMode }), signal });
  },
  ...
  track: (id: string, paramsOrSignal: { lyricPage?: number; lyricPageSize?: number; lyricCursor?: string; lyricCursorMode?: "cursor" | "offset" } | AbortSignal = {}, signal?: AbortSignal) => {
    const isSignal = typeof paramsOrSignal === "object" && "aborted" in paramsOrSignal;
    const params = isSignal ? {} : paramsOrSignal;
    const requestSignal = isSignal ? paramsOrSignal : signal;
    const options = Object.keys(params).length > 0 ? { query: query(params), signal: requestSignal } : { signal: requestSignal };
