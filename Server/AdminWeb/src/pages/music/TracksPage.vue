...
const bulk = reactive({ primary: "", albumArtists: "", album: "", genres: "", comment: "" });
...
const bulkMutation = useMutation({ mutationFn: () => { const tracks = [...selectedTracks.value.values()]; const archived = tracks.find((track) => track.status === "ARCHIVED"); if (archived) throw new Error(`已归档曲目“${archived.title}”需先恢复后才能批量修改`); const patch: Partial<Omit<TrackTagValues, "hasArtwork">> = {}; if (bulk.primary.trim()) patch.credits = nameList(bulk.primary).map((name) => ({ name, role: "PRIMARY" })); if (bulk.albumArtists.trim()) { const albumArtists = nameList(bulk.albumArtists); if (!albumArtists.length) throw new Error("请填写至少一个专辑艺术家"); patch.albumArtists = albumArtists; } if (bulk.album.trim()) patch.album = bulk.album.trim(); if (bulk.genres.trim()) patch.genres = genreList(bulk.genres); if (bulk.comment.trim()) patch.comment = bulk.comment.trim(); if (!Object.keys(patch).length) throw new Error("至少填写一个批量修改字段"); return musicAdmin.batchUpdateTrackMetadata(tracks, patch); }, onSuccess: async (result) => { allowBulkClose = true; bulkOpen.value = false; clearSelection(); ui.notify("success", `已更新 ${result.items.length} 首曲目`); await refresh(); }, onError: (error) => { actionError.value = error instanceof Error ? error.message : "批量修改失败"; } });
