...
<template>
  <div class="space-y-6 page-enter">
    ...
    <section v-if="duplicatesQuery.data.value?.duplicateAlbumCount" class="flex flex-col gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 sm:flex-row sm:items-center"><AlertTriangle :size="20" class="shrink-0 text-amber-500" /><div class="flex-1"><p class="font-bold">当前有 {{ duplicatesQuery.data.value.groupCount }} 组同名专辑，是否合并？</p><p class="mt-1 text-xs text-[var(--muted)]">共 {{ duplicatesQuery.data.value.duplicateAlbumCount }} 个可合并专辑；只按规范化后的专辑名称分组，不限制艺术家。</p></div><AppButton @click="duplicatesOpen = true"><template #icon><GitMerge :size="16" /></template>选择并合并</AppButton></section>
