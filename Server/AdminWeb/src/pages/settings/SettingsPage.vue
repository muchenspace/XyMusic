    ...
    <template v-else>
      <div class="grid gap-6 xl:grid-cols-[240px_minmax(0,1fr)]"><nav class="ui-card p-2"><button v-for="item in tabs" :key="item.key" type="button" class="pressable flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold" :class="tab === item.key ? 'bg-[var(--primary-soft)] text-[var(--primary)]' : 'text-[var(--muted)] hover:bg-[var(--surface-muted)]'" @click="tab = item.key; resetMessages()"><component :is="item.icon" :size="17" />{{ item.label }}</button></nav>
        <section class="ui-card overflow-hidden"><div class="border-b border-[var(--border)] px-5 py-5 sm:px-7"><h2 class="text-lg font-bold">{{ tabs.find((item) => item.key === tab)?.label }}</h2><p class="mt-1 text-xs text-[var(--muted)]">配置版本 {{ form.version }} · {{ settingsQuery.data.value?.environment }} · {{ settingsQuery.data.value?.configurationSource }}</p></div><div class="p-5 sm:p-7">
          ...
          <div v-else-if="tab === 'storage'" key="storage" class="grid gap-5 sm:grid-cols-2"><SettingField class="sm:col-span-2" label="媒体资产目录" :locked="locked('storage','assetDirectory')" hint="上传的音频源文件、封面及头像存储目录"><input v-model="form.storage.assetDirectory" class="ui-input font-mono" :disabled="locked('storage','assetDirectory')" /></SettingField><SettingField class="sm:col-span-2" label="转码临时目录" :locked="locked('storage','transcodeDirectory')" hint="即时转码临时音频片段存储目录"><input v-model="form.storage.transcodeDirectory" class="ui-input font-mono" :disabled="locked('storage','transcodeDirectory')" /></SettingField><SettingField label="单文件最大上传大小" :locked="locked('storage','maxUploadBytes')" :hint="formatBytes(form.storage.maxUploadBytes)"><input v-model.number="form.storage.maxUploadBytes" class="ui-input" type="number" min="1" :disabled="locked('storage','maxUploadBytes')" /></SettingField><SettingField label="转码缓存上限（字节）" :locked="locked('storage','transcodeCacheMaxBytes')" :hint="formatBytes(form.storage.transcodeCacheMaxBytes)"><input v-model.number="form.storage.transcodeCacheMaxBytes" class="ui-input" type="number" min="134217728" max="1099511627776" :disabled="locked('storage','transcodeCacheMaxBytes')" /></SettingField><SettingField label="最大并发转码任务数" :locked="locked('storage','streamMaxConcurrent')" hint="同时运行的最大 FFmpeg 实时转码进程数"><input v-model.number="form.storage.streamMaxConcurrent" class="ui-input" type="number" min="1" max="100" :disabled="locked('storage','streamMaxConcurrent')" /></SettingField><SettingField label="播放会话有效期（秒）" :locked="locked('storage','streamTtlSeconds')"><input v-model.number="form.storage.streamTtlSeconds" class="ui-input" type="number" min="30" max="86400" :disabled="locked('storage','streamTtlSeconds')" /></SettingField><SettingField label="转码超时（秒）" :locked="locked('storage','transcodeTimeoutSeconds')"><input v-model.number="form.storage.transcodeTimeoutSeconds" class="ui-input" type="number" min="5" max="3600" :disabled="locked('storage','transcodeTimeoutSeconds')" /></SettingField></div>
