...
// TestProductionStoreQueuesObsoleteLosslessVariantCleanup is deliberately
// opt-in because it writes PostgreSQL rows and creates a cleanup job.
func TestProductionStoreQueuesObsoleteLosslessVariantCleanup(t *testing.T) {
	environmentPath := os.Getenv("XYMUSIC_INTEGRATION_ENV")
	if environmentPath == "" {
		t.Skip("set XYMUSIC_INTEGRATION_ENV to run obsolete lossless variant cleanup")
	}
	testsupport.RequireWriteIntegration(t)
	absolutePath, err := filepath.Abs(environmentPath)
	if err != nil {
	...
	defer cancel()
	pctx, err := database.Open(ctx, cfg.Database)
	if err != nil {
		t.Fatalf("configured PostgreSQL is unavailable: %v", err)
	}
	t.Cleanup(pctx.Close)
	if err := database.CheckMigrationCompatibility(ctx, pctx.Pool, cfg.Paths.MigrationsDirectory); err != nil {
		t.Fatal(err)
	}
	store, err := NewPostgresStore(pctx.Pool)
	if err != nil {
		t.Fatal(err)
	}
	...
	sourceAssetID := uuid.NewString()
	oldAssetID := uuid.NewString()
	oldKey := "integration/mediaworker/" + suffix + "/old-lossless.flac"
	newKey := "integration/mediaworker/" + suffix + "/data_saver.m4a"
	attemptID := uuid.NewString()
	workerID := "media-store-integration-" + suffix
	cleanup := func() {
		cleanupContext, cleanupCancel := context.WithTimeout(context.Background(), 15*time.Second)
		defer cleanupCancel()
		_, _ = pctx.Exec(cleanupContext, `DELETE FROM object_cleanup_jobs WHERE object_key IN($1,$2)`, oldKey, newKey)
		_, _ = pctx.Exec(cleanupContext, `DELETE FROM media_jobs WHERE id=$1`, jobID)
		_, _ = pctx.Exec(cleanupContext, `DELETE FROM track_variants WHERE track_id=$1`, trackID)
		_, _ = pctx.Exec(cleanupContext, `DELETE FROM media_assets
			WHERE id=ANY($1::uuid[]) OR object_key IN($2,$3)`, []string{sourceAssetID, oldAssetID}, oldKey, newKey)
		_, _ = pctx.Exec(cleanupContext, `DELETE FROM tracks WHERE id=$1`, trackID)
	}
	t.Cleanup(cleanup)
	cleanup()

	transaction, err := pctx.Begin(ctx)
	if err != nil {
		t.Fatal(err)
	}
	seedFailed := true
	...
	if _, err := transaction.Exec(ctx, `INSERT INTO tracks(
		id,title,normalized_title,duration_ms,status,media_generation
	) VALUES($1,$2,$2,1000,'READY',1)`, trackID, "Obsolete Lossless "+suffix); err != nil {
		t.Fatal(err)
	}
	if _, err := transaction.Exec(ctx, `INSERT INTO media_assets(
		id,object_key,kind,mime_type,size_bytes,checksum_sha256,status
	) VALUES
		($1,$2,'AUDIO_SOURCE','audio/mp4',100,$3,'READY'),
		($4,$5,'AUDIO_VARIANT','audio/flac',100,$6,'READY')`,
		sourceAssetID, "integration/mediaworker/"+suffix+"/source.m4a", strings.Repeat("a", 64),
		oldAssetID, oldKey, strings.Repeat("b", 64)); err != nil {
		t.Fatal(err)
	}
	if _, err := transaction.Exec(ctx, `INSERT INTO media_jobs(
		id,type,source_asset_id,track_id,status,generation,idempotency_key,payload,
		locked_by,attempt_id,cancel_requested,publish_on_ready
	) VALUES($1,'INGEST_TRACK',$2,$3,'PROCESSING',1,$4,'{}'::jsonb,$5,$6,false,true)`,
		jobID, sourceAssetID, trackID, "obsolete-lossless-"+suffix, workerID, attemptID); err != nil {
		t.Fatal(err)
	}
	if _, err := transaction.Exec(ctx, `INSERT INTO track_variants(
		track_id,asset_id,quality,mime_type,codec,container,bitrate,status
	) VALUES($1,$2,'LOSSLESS','audio/flac','flac','flac',1,'READY')`, trackID, oldAssetID); err != nil {
		t.Fatal(err)
	}
	if err := transaction.Commit(ctx); err != nil {
		t.Fatal(err)
	...
	profile := AudioVariantProfiles("aac")[0]
	replaced, err := store.CommitMediaJob(ctx, CommitMediaJob{
		Job:      MediaJob{ID: jobID, SourceAssetID: sourceAssetID, TrackID: trackID, Generation: 1, PublishOnReady: true, AttemptID: &attemptID},
		WorkerID: workerID, DurationMS: 1000,
		Generated: []GeneratedVariant{{
			Profile: profile, ObjectKey: newKey, SizeBytes: 100,
			ChecksumSHA256: strings.Repeat("c", 64), SourceChecksumSHA256: strings.Repeat("a", 64), ProfileVersion: "v1",
		}},
	...
	if len(replaced) != 1 || replaced[0] != oldAssetID {
		t.Fatalf("replaced assets = %#v, want [%s]", replaced, oldAssetID)
	}
	if err := store.ScheduleReplacedAssetCleanup(ctx, replaced, time.Now().UTC()); err != nil {
		t.Fatal(err)
	}
	var oldStatus, cleanupStatus string
	if err := pctx.QueryRow(ctx, `SELECT status::text FROM media_assets WHERE id=$1`, oldAssetID).Scan(&oldStatus); err != nil {
		t.Fatal(err)
	}
	if err := pctx.QueryRow(ctx, `SELECT status::text FROM object_cleanup_jobs WHERE object_key=$1`, oldKey).Scan(&cleanupStatus); err != nil {
		t.Fatal(err)
	}
	if oldStatus != "DELETE_PENDING" || cleanupStatus != "PENDING" {
		t.Fatalf("obsolete lossless asset=%s cleanup=%s", oldStatus, cleanupStatus)
	}
}
