...
func TestGinRoutesCoverEveryLegacyAPI(t *testing.T) {
	if os.Getenv("XYMUSIC_REQUIRE_FULL_API_PARITY") != "1" {
		t.Skip("set XYMUSIC_REQUIRE_FULL_API_PARITY=1 for the final 133-endpoint gate")
	}
	...
	if len(differences) != 0 {
		...
		t.Fatalf("Gin API coverage differs from the 133-endpoint manifest (%d registered, %d expected):\n%s", len(actual), len(expected), strings.Join(differences, "\n"))
	}
}
