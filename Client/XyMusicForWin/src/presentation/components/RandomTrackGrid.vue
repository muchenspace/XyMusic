...
<template>
  <section class="content-section random-tracks-section" aria-label="猜你喜欢">
    ...
    <div class="random-track-grid">
      <article v-for="track in tracks" :key="track.id" v-memo="[track, track.liked, currentId === track.id, currentId === track.id ? isPlaying : false]" class="random-track-card" :class="{ current: currentId === track.id }">
