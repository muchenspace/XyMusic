...
<template>
  <footer v-if="player.currentTrack" class="lyrics-player-controls" aria-label="歌词页播放器">
    ...
    <div class="lyrics-transport">
      ...
      <PlaybackProgress show-times class="lyrics-progress-row" />
    </div>
    <div class="lyrics-player-extras">
      ...
      <input :value="player.volume" aria-label="音量" type="range" min="0" max="100" :aria-valuetext="`${Math.round(player.volume)}%`" :style="{ '--range-progress': `${player.volume}%` }" @input="updateVolume" @change="player.flushPlayerPreferences" />
    </div>
  </footer>
</template>
